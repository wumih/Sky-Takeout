...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderPaymentVO payment(OrdersPaymentDTO ordersPaymentDTO) throws Exception {
        ...
        //         //调用微信支付接口，生成预支付交易单。由于没有微信商户资质所以只能先注释掉了
        ...
        // 模拟支付：跳过真实微信支付调用，直接修改订单状态为已支付
        // 因为没有微信支付商户号和证书，无法调用真实的微信支付接口
        // 所以直接调用 paySuccess 将订单状态改为"待接单"、支付状态改为"已支付"
        paySuccess(ordersPaymentDTO.getOrderNumber());

        // 返回空的 OrderPaymentVO（前端会根据返回结果进行处理）
    ...
    public void userCancelById(Long id) throws Exception {
        ...
        if (ordersDB.getStatus().equals(Orders.TO_BE_CONFIRMED)) {
            // 模拟退款（实际项目中应调用微信退款接口）
            log.info("用户取消订单，模拟退款：{}", ordersDB.getNumber());
            // 设置支付状态为退款
     ...
     * @return
     * 作用	整个订单搜索功能的入口和主流程编排器
     * 它做了什么	① 设置分页 → ② 调 Mapper 查数据库 → ③ getOrderVOList 转换数据 → ④ 包装返回结果
     *
     */
    ...
    /**
     * 把数据库查出来的Orders列表批量转换为前端需要的OrderVO列表
     *
     * @param page 分页查询结果
     * @return OrderVO列表
     */
    ...
    public void reminder(Long id) {
        ...
        // 通过 WebSocket 向商家端推送催单消息（type=2）
        Map map = new HashMap();
        map.put("type", 2); // 消息类型：2 表示客户催单
        map.put("orderId", id);
